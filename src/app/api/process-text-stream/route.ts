...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📤 Body enviado a upstream /api/process-text-stream:', JSON.stringify({ prompt,context
    }, null, 2));
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          console.log('📥 Upstream respuesta (process-text-stream) status:', upstreamResponse.status);
          ...
          console.error('Error en streaming (process-text-stream):', error);
    ...
    console.error('Error in process-text-stream API route:', error);
